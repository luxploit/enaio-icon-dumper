...
const appConnectorUrl = "/osrest/api/icon/ids";
let baseUrl = "";
...
async function fetchIcons() {
	...
	/* rip off AppConnector (!DEPRECATED!) whilst I still can */
...
//* Usage: npm start -- "https://<enaio base url>"
const main = async () => {
	if (process.argv.length() < 2) {
		console.error("No BaseURL provided!");
		process.exit(1);
	} else {
		baseUrl = process.argv.at(1); // first argument
	}
